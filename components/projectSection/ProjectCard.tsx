...
export function ProjectCard({
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-full border border-border bg-black text-white rounded-xl overflow-hidden hover:ring-2 cursor-pointer hover:ring-muted transition-all duration-200",
        className
      )}
    >
        ...
        {tags && tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-auto pt-4">
            {tags.map((tag) => (
              <Badge
                key={tag}
                className="text-[11px] font-medium border border-border h-6 w-fit px-2 bg-slate-900 text-slate-300"
                variant="outline"
              >
                {tag}
              </Badge>
